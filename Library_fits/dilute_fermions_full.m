...
for num_funcs = 1:max_num_funcs
    ...
    lb = [-25*ones(num_funcs,1);zeros(num_funcs,1)];%lower boundary
    
    if exist(['Dilute_fermions/vec_fermi_TS_' num2str(zF) '_' num2str(num_funcs) '.txt'], 'file')
            vec = load(['Dilute_fermions/vec_fermi_TS_' num2str(zF) '_' num2str(num_funcs) '.txt']);
    else
        [vec,error_fit] = get_Taylor_vec_fit(zF,num_funcs,xdata,ydata,lb,ub,options,flag);
        disp(['Error_fit = ' num2str( error_fit )]);
        writematrix(vec,['Dilute_fermions/vec_fermi_TS_' num2str(zF) '_' num2str(num_funcs) '.txt'])
    end
end
disp('FERMIONS ARE COMPLETE')
...
for num_funcs = 1:max_num_funcs
    ...
    lb = [-20*ones(num_funcs,1);zeros(num_funcs,1)];
    if num_funcs == 1
        vec = load(['Dilute_fermions/vec_fermi_TS_' num2str(zF) '_' '1.txt']);
        writematrix(vec,['Dilute_fermions/vec_fermi_' num2str(zF) '_' num2str(num_funcs) '.txt']);
        Error_list(1) = max(abs(ydata - poly_Gauss_approx(vec,xdata)));
    else
        if ~exist(['Dilute_fermions/vec_fermi_' num2str(zF) '_' num2str(num_funcs) '.txt'], 'file')%it loads previous best
            vec_prev = load(['Dilute_fermions/vec_fermi_' num2str(zF) '_' num2str(num_funcs - 1) '.txt']);
...
%         %4) we add a not so random Gaussian to vec_prev, then optimize
            vec_1 = load(['Dilute_fermions/vec_fermi_TS_' num2str(zF) '_' num2str(num_funcs) '.txt']);
            ...
            vec_4 = [amps_prev;(-1)^(num_funcs + 1);stds_prev;1/sqrt(num_funcs)];
            ...
            writematrix(vec,['Dilute_fermions/vec_fermi_' num2str(zF) '_' num2str(num_funcs) '.txt'])
            writematrix(Error_list,['Dilute_fermions/Error_list_' num2str(zF) '.txt'])
            disp(['Ongoing error = ' num2str(error_best) '; number of Gaussians = ' num2str(num_funcs)])
        else
            vec = load(['Dilute_fermions/vec_fermi_'  num2str(zF) '_' num2str(num_funcs) '.txt']);
